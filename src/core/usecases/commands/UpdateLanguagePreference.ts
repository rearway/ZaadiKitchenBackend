...
export function makeUC(deps: Deps) {
    return async function updateLanguagePreference(input: UpdateLanguagePreferenceInput): Promise<UpdateLanguagePreferenceOutput> {
        ...
        try {
            ...
            logger.error('Failed to update language preference', error instanceof Error ? error.message : String(error))
            throw error
        }
    }
}
