...
export function makeUC(deps: Deps) {
    return async function submitOutOfZoneInterest(input: SubmitOutOfZoneInterestInput): Promise<SubmitOutOfZoneInterestOutput> {
        ...
        try {
            ...
            if (!areaName || areaName.trim().length < 2) {
                ...
                throw new ValidationError('AREA_NAME_REQUIRED', { message: 'Area name cannot be empty' })
            }

            const pastSubmissions = await outOfZoneInterestPersistor.getUserInterestCount(userId)
            if (pastSubmissions >= 3) {
                const { BaseError } = await import('../../../shared/errors/index.js')
                throw new BaseError('SUBMISSION_LIMIT', 429, 'Max 3 out-of-zone submissions per session')
            }
            ...
            logger.error('Failed to submit out of zone interest', error instanceof Error ? error.message : String(error))
            throw error
        }
    }
}
