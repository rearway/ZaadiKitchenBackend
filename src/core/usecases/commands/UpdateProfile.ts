...
export function makeUC(deps: Deps) {
    return async function updateProfile(input: UpdateProfileInput): Promise<UpdateProfileOutput> {
        ...
        try {
            ...
            const onboardingComplete = !!updatedUser.fullName && updatedUser.fullName !== 'New User'
            ...
            logger.error('Failed to update profile', error instanceof Error ? error.message : String(error))
            throw error
        }
    }
}
