...
export function makeUC(deps: Deps) {
  return async function getBuildingsForArea(
    input: GetBuildingsForAreaInput
  ): Promise<GetBuildingsForAreaOutput> {
    ...
    try {
      ...
      if (query && query.trim() !== '') {
        buildings = await buildingLoader.searchBuildingsByArea(
          areaId,
          query.trim()
        )
      } else {
      ...
      logger.error(
        'Failed to get buildings for area',
