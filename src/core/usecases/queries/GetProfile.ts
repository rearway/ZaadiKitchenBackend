...
export function makeUC(deps: Deps) {
    return async function getProfile(input: GetProfileInput): Promise<GetProfileOutput> {
        ...
        try {
            ...
            logger.error('Failed to get profile', error instanceof Error ? error.message : String(error))
            throw error
        }
    }
}
